...
>根据官方描述，ESP32 C3和S3都内置JTAG调试器，即我们只需要通过USB先连接到32的USB引脚，就能通过IDE直接调试，不需要额外的如ST-Link或Jtag调试硬件工具，非常方便。
>同时注意，使用调试功能，USB引脚就不能用于其它功能了。除非后续不再使用调试功能，那usb两个引脚可以作为他用。
