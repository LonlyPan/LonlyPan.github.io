...
2. Fifo Implementation（FIFO 实现)：资源核时钟的组合=FIFO的实现
	1. 资源：Block RAM（块 RAM）、Distributed RAM（分布式 RAM）、Shift Register（移位寄存器）和 Builtin FIFO（内置 FIFO），其中移位寄存器仅可用于实现同步 FIFO。可配置 的 FIFO 类型有两类，分别为 Common Clocks（公共时钟，即同步 FIFO）和 Independent Clocks（独立时 钟，即异步 FIFO）。资源与种类两两组合之下便有了七种不同的选择。
