...
1. Monitor Type：ILA 探针接口类型设置。Native 通常是用来测量电平或一定位宽信号，AXI 直接测量 AXI 总线的信号。
	1. Number of Probes：探针数量设置，在 GUI 界面最大可设置 64 个，可以通过 TCL 脚本去生成 IP Core 超过 64 个，或者可以通过生成多个 ILA IP Core 去调试更多信号。**每个探针不是指一个信号，而是一g组**。比如AD的14位数据，就只需要一个探针。每个探针还可以设置位宽，从而测量多个同类信号。
