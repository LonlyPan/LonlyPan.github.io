...
有个问题，就是LED3这个灯在复位后总会闪烁两次，这个是只要接上核心板就会有该问题（不论有没有烧程序），目前推测是硬件问题，暂不影响本次实验。
...
void led_init(void)
{
	...
	IOMUXC_SetPinConfig(IOMUXC_SNVS_SNVS_TAMPER5_GPIO5_IO05,0X10B0);
	/* 3、初始化GPIO,设置GPIO1_IO03设置为输出  */
	GPIO1->GDIR |= (1 << 9);	
	GPIO5->GDIR |= (1 << 9);	
	GPIO5->GDIR |= (1 << 5);	
...
## HC595实验-Forlinx
Forlinx没有蜂鸣器，有一个HC595串转并电路，这里测试以下。
...
- SER（Serial Input，串行输入）引脚，也就是数据引脚
- SRCLK（移位寄存器时钟）是移位寄存器的时钟。 在上升沿时，将SER引脚的数据（高低电平）移入移位寄存器
...
74HC595内部有一个8位的移位寄存器用来保存从DS引脚输入的数据。那么74HC595怎么知道什么时候该从SER引脚上取数据了呢？正是通过SRCLK这个时钟引脚来实现的。只有在SRCLK发生一次上升沿的时候，74HC595才会从SER引脚上取得当前的数据（高/低电平）并把取到的这一位数据保存到移位寄存器里。当我们向芯片发送数据时，要先在SER引脚上准备好要传送的数据，然后制造一次SRCLK引脚的上升沿（先拉低电平再拉高电平），74HC595会在这个上升沿将SER引脚上的数据存入移位寄存器D0，同时D0原来的数据会顺移到D1，D1的数据位移到D2。。。D6的数据位移到D7。而原先D7的数据已经没有地方储存了，这一位数据会被输出到引脚QH'上。（注意这里说的不是输出引脚Q0-Q7，而是指内部的8位移位寄存器里每一个“小房间”，芯片手册上并没有给这些小房间编号，这里为了说明方便进行了编号）
RCLK是芯片内部另外一个8位锁存寄存器的时钟引脚。当移位寄存器的8位数据全部传输完毕后，制造一次锁存器时钟引脚的上升沿（先拉低电平再拉高电平）。74HC595会在这个上升沿将移位寄存器里的8位数据复制到锁存器中（锁存器里原来的数据将被替换）。注意，到这里为止，这8位数据还只是被保存在锁存器里，并没有输出到数码管上。这个引脚同样连接到树莓派任意一个GPIO口上即可。
OE（输出使能）也是负逻辑，低电压时，锁存器里数据才能被实时输出到QA–QH引脚。



![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/74hc595.gif)
...
|  芯片引脚名   |   原理图名称  |  功能 |
| --- | --- |--- |
|  BOOT_MODE0   |  SHIFT_SDI   |  数据输入 |
|   BOOT_MODE1  |  SHIFT_SHCP   | 移位寄存器时钟|
...
> CCM_ANALOG->PLL_ARM：div_select值范围：54-108. Fout = Fin（24MHz） * div_select/2.0.同时Fout ( PLL1即PLL_ARM) - 最大 1.3 GHz.
> CACRR：Divider for ARM clock root. 分频系数，1-8（0-7）
...
void imx6u_clkinit(void)
{
	 ...
	 *      配置CCM_ANLOG->PLL_ARM寄存器
	 *      bit13: 1 使能时钟输出
	 *      bit[6:0]: 66, 由公式：Fout = Fin * div_select / 2.0，1065=24*div_select/2.0,
	 *              		得出：div_select=    88 
	 */
	CCM_ANALOG->PLL_ARM = (1 << 13) | ((88 << 0) & 0X7F); 	/* 配置pll1_main_clk=792MHz */
	CCM->CCSR &= ~(1 << 2);									/* 将pll_sw_clk时钟重新切换回pll1_main_clk */
	CCM->CACRR = 1;											/* ARM内核时钟为pll1_sw_clk/1=1065/2=528Mhz */
...
如果我们不加额外的延时 2.5s ，屏幕驱动就会有问题（如下所示），花了两个晚上排查，才推测是需要延时
...
我们接下来看一下 I2C 的几个重要的寄存器，首先看一下 
- I2Cx_IADR(x=1~4)寄存器，这是I2C 的地址寄存器，只有 ADR(bit7:1)位有效，用来保存 I2C 从设备地址数据。当我们要访问某个 I2C 从设备的时候就需要将其设备地址写入到 ADR 里面。
- 寄存器I2Cx_IFDR，这个是 I2C 的分频寄存器，只有 IC(bit5:0)这个位，用来设置 I2C 的波特率，I2C 的时钟源可以选择 IPG_CLK_ROOT=66MHz，通过设置 IC 位可以得到想要的 I2C 波特率。IC 位可选的设置
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698328424998.png)
- 寄存器 I2Cx_I2CR，这个是 I2C 控制寄存器，寄存器 I2Cx_I2CR 的各位含义如下：
...
- 寄存器 I2Cx_I2SR，这个是 I2C 的状态寄存器，寄存器结构如图 26.1.2.5 所
...
-  I2Cx_I2DR，这是 I2C 的数据寄存器，此寄存器只有低 8 位有效，当要发送数据的时候将要发送的数据写入到此寄存器，如果要接收数据的话直接读取此寄存器即可得到接收到的数据。
...
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698328690007.png)
...
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698328721825.png)
0X00 这个寄存器是模式控制寄存器，用来设置 AP3216C 的工作模式，一般开始先将其设置为 0X04，也就是先软件复位一次 AP3216C。接下来根据实际使用情况选择合适的工作模式，比如设置为 0X03，也就是开启 ALS+PS+IR。
 0X0A~0X0F 这 6 个寄存器就是数据寄存器，保存着 ALS、PS 和 IR 这三个传感器获取到的数据值。如果同时打开 ALS、PS 和 IR 则读取间隔最少要 112.5ms，因为 AP3216C 完成一次转换需要 112.5ms。

开机会先检测 AP3216C 是否存在，一般的芯片是有个 ID 寄存器，通过读取 ID 寄存器判断 ID 是否正确就可以检测芯片是否存在。但是 AP3216C 没有 ID 寄存器，所以我们就通过向寄存器 0X00 写入一个值，然后再读取 0X00 寄存器，判断读出得到值和写入的是否相等，如果相等就表示 AP3216C 存在，否则的话 AP3216C 就不存在。本章的配置步骤如下：
1、初始化相应的 IO
...
保留位0x17H写入固定值0xD8
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698847391793.png)

保留位0x31H写入固定值0x08
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698847425683.png)
...
## Linux系统移植概念
 ...
 Linux操作系统向下管理硬件（I/O，设备接口），向上提供接口（进程管理+文件IO+网络协议+数据库等，被APP软件调用
 
bootloader（U-Boot） -> Linux内核 -> 根文件系统(rootfs) 。这三者一起构成了一个完整的Linux系统，一个可以正常使用、功能完善的Linux系统。

开发板上电后首先运行 SOC 内部 iROM 中固化的代码（BL0），这段代码先对基本的软硬件环境（时钟等）初始化，然后检测拨码开关获取启动方式，再将对应存储器（SD卡）中的Uboot搬移到内存，然后跳转到uboot运行
...
现成的bootloader软件有很多，比如U-Boot、vivi、RedBoot等等，其中以U-Boot使用最为广泛，为了方便书写，本书会将U-Boot写为uboot。uboot一般分为三种：
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1700143628792.png)

最常用的就是半导体厂商或者开发板厂商的 uboot，如果你用的半导体厂商的评估板，那么就使用半导体厂商的 uboot，如果你是购买的第三方开发板，比如正点原子的 I.MX6ULL 开发板，就使用正点原子提供的 uboot 源码（也是在半导体厂商的 uboot 上修改的）。
当然了，你也可以在购买了第三方开发板以后使用半导体厂商提供的 uboot，只不过有些外设驱动可能不支持，需要自己移植，这个就是我们常说的 uboot 移植。

NXP官方uboot下载地址github：https://github.com/nxp-imx
下载rel_imx_4.1.15_2.1.0_ga版本。
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/image_2_.jpg)
...
修改Linux文件后，不能直接外部断电或复位重启，此时部分文件还在运行缓存中，弱国要重启，最好使用命令 `reboot`执行重启，系统会自动保存缓存数据后再重启。或者可以使用 `sync` 命令强制保存。

否则直接外部重启或断电会导致修改的数据丢失，比如刚修改的文件不会被修改，还保持原样。

>系统在运行时，会自动后台不定时保存缓存内容。所以如果怕丢失重要数据，就每次修改后 `sync`强制更新
