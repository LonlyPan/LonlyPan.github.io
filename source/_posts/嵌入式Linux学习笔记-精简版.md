...
- SRCLR（移位寄存器清除）引脚允许我们一次复位整个移位寄存器，使其所有位为0。这是一个负逻辑引脚，因此要执行复位，我们需要将SRCLR引脚设置为低电平。当不需要复位时，该引脚应为高电平。
- OE（输出使能）也是负逻辑：当其上的电压为高时，输出引脚被禁用/设置为高阻抗状态并且不允许电流流动。当OE低电压时，输出引脚正常工作。
- QA–QH（输出使能）是输出引脚，应连接到某种类型的输出，如LED、7段数码管等。
- QH' 引脚输出移位寄存器的第7位。SER的数据最终还会从该引脚输出，也就是SER数据怎么输入，QH' 就怎么输出。但要会必SER数据落后8个时钟周期。可以利用该引脚级联到下一个HC595的SER引脚，组合使用。

#### 74HC595移位寄存器如何工作？

74HC595内部有一个8位的移位寄存器用来保存从SER引脚输入的数据。那么74HC595怎么知道什么时候该从DS引脚上取数据了呢？正是通过SRCLK这个时钟引脚来实现的。只有在SRCLK发生一次上升沿的时候，74HC595才会从SER引脚上取得当前的数据（高/低电平）并把取到的这一位数据保存到移位寄存器里。当我们向芯片发送数据时，要先在SER引脚上准备好要传送的数据，然后制造一次SRCLK引脚的上升沿（先拉低电平再拉高电平），74HC595会在这个上升沿将DS引脚上的数据存入移位寄存器D0，同时D0原来的数据会顺移到D1，D1的数据位移到D2。。。D6的数据位移到D7。而原先D7的数据已经没有地方储存了，这一位数据会被输出到引脚QH' 上。（注意这里说的不是输出引脚Q0-Q7，而是指内部的8位移位寄存器里每一个“小房间”，芯片手册上并没有给这些小房间编号，这里为了说明方便进行了编号）

RCLK是芯片内部另外一个8位锁存寄存器的时钟引脚。当移位寄存器的8位数据全部传输完毕后，制造一次锁存器时钟引脚的上升沿（先拉低电平再拉高电平）。74HC595会在这个上升沿将移位寄存器里的8位数据复制到锁存器中（锁存器里原来的数据将被替换）。此时如果OE使能了，数据就会被输出到QA–QH引脚上。
...
![enter description here](https://lonly-hexo-img.oss-cn-shanghai.aliyuncs.com/hexo_images/嵌入式Linux学习笔记-精简版/1698577056505.png)
