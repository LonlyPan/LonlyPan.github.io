...
中断向量表通常位于内存的固定地址上，处理器会在启动时加载中断向量表的地址。当发生一个中断事件时，处理器会根据中断号从中断向量表中读取对应的中断处理程序地址，并开始执行该程序。
...
因此根据上面代码来说，系统上电，从_start:开始执行程序，并加载中断向量表的地址。
当系统发生中断后 跳转到对应的中断位置，改地址上，是一条跳转指令，则会跳去执行对应的函数(地址)。
...
因此，中断向量表是操作系统和硬件之间的一个重要接口。操作系统可以通过修改中断向量表中的入口地址来安装自己的中断处理程序，从而改变中断处理的行为。同时，硬件厂商可以通过修改中断向量表中的入口地址来实现自己的中断处理机制。
...
为了解决这个问题， Cortex-M 架构引入了一 个新的概念——中断向量表偏移。通过中断向量表偏移就可以将中断向量表存放到任意地址处，STM32的中断向量表偏移配置在函数 SystemInit 中完成，通过向 SCB_VTOR 寄存器写入新的中断 向量表首地址即可。
