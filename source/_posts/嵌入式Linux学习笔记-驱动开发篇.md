...
platform设备驱动又叫平台设备驱动。在一个产品中，大量设备均直接属于片上系统（SOC、芯片），如UART、以太网、SPI等。platform专门用于处理除了UART、以太网、SPI等外的平台设备（如LED、LCD等），大体与普通设备类似，但platform设备是被静态枚举而不是动态。

> platform 设备驱动和普通设备驱动一样，不过是专门针对片上系统（SOC）写的，。也有专门的语法。
