...
事件：自己有一电钢琴，平时练习，会使用 耳机线（双公头那种延长线） 将钢琴的耳机输出接到电脑的麦克风输入口中，从而录制钢琴的声音。之前使用一直没问题。但最近，只要将耳机线头插入电脑的麦克风口（另一端不接钢琴耳机口），OBS（录制视频的软件）中的麦克风音量显示处就会有杂音，而且分贝值很高。下图红框中绿色线条部分就是实时显示输入的音量大小，但实际并没有任何输入，另一头还没接钢琴。
![enter description here](https://LonlyPan.github.io/images/Posts/2002-07-10-电脑PC相关/音频输入采集.png)

解决：瞎试的，并不是确定是否有效，但自己的成功了。

1. Win10系统，右键单击右下角`喇叭图标`，单击`声音`
2. 弹窗中选择`录制`，选中你的麦克风口（不知道那个，就把除了现在使用的其余音频口都拔了，这里就只会显示一个可选，我这就只有一个），然后单击`属性`进入属性配置界面
3. 在`设备用法中`选择`不使用此设备`。单击确认退出
4. 重新插拔耳机口
5. 再回到第三步的位置，选择`使用此设备`，重新打开，单击确认退出。然后就莫名的好了、、、
![enter description here](https://LonlyPan.github.io/images/Posts/2002-07-10-电脑PC相关/音频设置.png)