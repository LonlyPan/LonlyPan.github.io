...
事件：自己有一电钢琴，平时练习，会使用 耳机线（双公头那种延长线） 将钢琴的耳机输出接到电脑的麦克风输入口中，从而录制钢琴的声音。之前使用一直没问题。但最近，只要将耳机线头插入电脑的麦克风口（另一端不接钢琴耳机口），OBS（录制视频的软件）中的麦克风音量显示处就会有杂音，而且分贝值很高。下图红框中绿色线条部分就是实时显示输入的音量大小，但实际并没有任何输入，另一头没接钢琴。
