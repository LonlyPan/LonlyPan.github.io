...
> 模块地址：真正意义上的软件地址，也就是通过设置后，模块的程序会识别该参数
> 信道地址：实际是和传输频率挂钩的，也就是信道地址其实是代表了模块的通信频率（160-173MHz）

**定点发射：**
在定点发射下，每个用户数据帧的前3个字节作为模块地址（0、1字节）、 信道（3字节）。 发射时， 模块改变自身地址和信道， 完毕后， 恢复原有设置。
>如何理解后一句话？
>每个模块都有一个自身的地址和信道。如果再定点模式模式下

**广播发射（透明传输）：**
广播发射下，用户数据是不需要额外添加数据帧的（定点模式需要添加）。只要收发双方的信道地址相同（默认：0x28），其实就是模块的通信频率一致，，就能够通信。但是如果信道不同，则无法通信，此时必须使用定点模式。

总结：
传输模式设置的是模块再发射时的状态（从模式名字就可以看出来，都是发射），如果是作为接收方就收数据时，则此位设置无效。
>该模块时收发一体的，既可以作为发射数据，也可用来接收数据、
只要模块设置位定点模式，发送数据时就必须设置数据帧，否则接收方无法接收数据。通信过程与接收方的模式设置无关
