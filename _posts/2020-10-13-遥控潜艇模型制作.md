...
通过修改参数 `OPTION` 的第 8 位，可以设置模块的传输模式：定点发射或广播发射（透明传输）。  
...
**广播发射（透明传输）：**
广播发射下，用户数据是不需要额外添加数据帧的（定点模式需要添加）。只要收发双方的信道地址相同（默认：0x28），其实就是模块的通信频率一致，就能够通信。但是如果信道不同，则无法通信。

**总结：**
传输模式设置的是模块在发射时的状态（从模式名字就可以看出来，都是发射），如果是作为接收方，则此位设置无效。
- 只要模块设置为定点模式，发送数据时就必须设置数据帧，否则接收方无法接收数据。通信过程与接收方的模式设置无关
- 只要模块设置为透明模式，发送数据无需数据帧，发送多少，接收方收多少（就算加了数据帧，也只会作为数据传输，不会被模块识别）。通信过程与接收方的模式设置无关

那么其实这里就有一个疑问了，既然传输模式设置和接收方无关，那么是不是任何一个工作在透明模式下的发射模块发送数据，相同信道下的接收方都能接收到数据。
如果有两个遥控器，一个接收器。那么信道相同下，其实时可以两个遥控器控制一个接收器的。这里就可能出现混乱。
为了避免混乱：一是将发射器都设置在顶点模式下，二是将信道改变。三其实靠我们自己在数据中加识别码，自己通过软件识别。
