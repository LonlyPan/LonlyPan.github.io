...
> 模块地址是真正意义上的软件地址，也就是通过设置后，模块的程序会识别该参数
> 信道地址：实际是和传输频率挂钩的，也就是信道地址其实是代表了模块的通信频率（160-173MHz）

**定点发射：**
在定点发射下，每个用户数据帧的前3个字节作为模块地址（0、1字节）、 信道（3字节）。 发射时， 模块改变自身地址和信道， 完毕后， 恢复原有设置。
>如何理解后一句话？
>每个模块都有一个自身的地址和信道。如果再顶点模式下

**广播发射（透明传输）：**
广播发射下，用户数据是不需要额外添加数据帧的（定点模式需要添加）。只要收发双方的信道地址相同（默认：0x28），模块地址可以不同，就能够通信。但是如果信道不同，则无法通信，此时必须使用定点模式。
#### 工作模式
