...
1、模拟配置会关闭引脚的上拉、下拉和施密特触发器。使得引脚功耗为0。因此可以通过将所有引脚配置为该模式来**降低功耗**。
2、模拟配置另外好处就是保证了这个引脚是“干净”的，如果和外部连接，那个该引脚就完全反映了外部引脚状态。因此将该引脚连接到A/D 片上外设，就可以测量引脚的模拟值了。所以我们将引脚复用为ADC功能时，就会默认设置为Analog模式。
