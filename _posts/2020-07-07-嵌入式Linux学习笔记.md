...
接下来详细的看一下CCM_CACRR和CCM_ANALOG_PLL_ARMn这两个寄存器
- 寄存器CCM_CACRR只有ARM_PODF位，可以设置为0~7，分别对应1~8分频。如果要设置为2分频的话CCM_CACCR就要设置为1。
- 再来看一下寄存器CCM_ANALOG_PLL_ARMn，此寄存器结构如图16.1.4.3所示：