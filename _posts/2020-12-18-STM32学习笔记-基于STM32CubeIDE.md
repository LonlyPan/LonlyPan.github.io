...
STM32CubeIDE 配置界面提示，要想生成  48MHz 时钟，必须使用外部晶振作为时钟源，但芯片数据手册并未对此做说明，参考网上资料，使用内部时钟源也是可以的，只是时钟精度不高，导致数据传输不稳定。介于此，还是乖乖使用外部时钟。
...
PQ 是 P、Q分频前的时钟
pq对比的是已经提前筛选的 P、Q分频前的时钟，可以产生 >=72Mhz 的系统时钟。其它可能值已经剔除。
...
int main()
{
   /* 我的第一个 C 程序 */
   printf("Hello, World! \n");
   for(int M = 12;M <64;M++){  // M分频前时钟必须在 0.91-2.1MHz 之间
      for(int N = 50;N <433;N++){
		float pq = (float)25/M*N;  // 计算得到P、Q分频前的时钟
		if(pq == 144)
			 printf("M = %d,N = %d,HCLK = 72,PQ = %f\n",M,N,pq);
		else if(pq == 288)
			printf("M = %d,N = %d,HCLK = 72,PQ = %f\n",M,N,pq);
		else if(pq == 336)
			 printf("M = %d,N = %d,HCLK = 84,PQ = %f\n",M,N,pq);
		else if(pq == 432)
			 printf("M = %d,N = %d,HCLK = 72,PQ = %f\n",M,N,pq);
		else if(pq == 480)
			 printf("M = %d,N = %d,HCLK = 80,PQ = %f\n",M,N,pq);
		else if(pq == 576)
			 printf("M = %d,N = %d,HCLK = 72,PQ = %f\n",M,N,pq);
		else if(pq == 624)
			 printf("M = %d,N = %d,HCLK = 78,PQ = %f\n",M,N,pq);
		else if(pq == 672)
			 printf("M = %d,N = %d,HCLK = 84,PQ = %f\n",M,N,pq);
	  }
   }
